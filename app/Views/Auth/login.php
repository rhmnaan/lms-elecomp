...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        html, body { height: 100%; font-family: 'DM Sans', sans-serif; background: var(--blue-deep); overflow: hidden; overflow-y: auto; }
        ...
        .bg-canvas::before { content: ''; position: absolute; inset: 0; background-image: radial-gradient(circle at 20% 80%, rgba(3,170,222,.18) 0%, transparent 50%), radial-gradient(circle at 80% 20%, rgba(0,229,192,.12) 0%, transparent 45%), radial-gradient(circle at 50% 50%, rgba(13,38,86,.6) 0%, transparent 70%); }
        ...
        @keyframes particleDrift { 0% { transform: translateY(100vh) translateX(0); opacity: 0; } 10% { opacity: 1; } 90% { opacity: 1; } 100% { transform: translateY(-10vh) translateX(40px); opacity: 0; } }
        ...
        @keyframes pulse { 0%,100% { opacity: 1; transform: scale(1); box-shadow: 0 0 0 0 rgba(0,229,192,.5); } 50% { opacity: .8; transform: scale(1.3); box-shadow: 0 0 0 6px transparent; } }
        .hero-title { font-family: 'Syne', sans-serif; font-size: clamp(38px,4vw,56px); font-weight: 800; line-height: 1.05; color: var(--white); margin-bottom: 18px; }
        ...
        .hero-sub { font-size: 15px; line-height: 1.7; color: rgba(255,255,255,.5); max-width: 360px; margin-bottom: 40px; }
        .feature-pills { display: flex; flex-wrap: wrap; gap: 10px; }
        .pill { display: flex; align-items: center; gap: 8px; background: rgba(255,255,255,.05); border: 1px solid rgba(255,255,255,.08); border-radius: 100px; padding: 8px 14px; font-size: 13px; color: rgba(255,255,255,.6); backdrop-filter: blur(4px); transition: all .25s; }
        .pill i { color: var(--blue-sky); font-size: 12px; }
        .pill:hover { background: rgba(3,170,222,.1); border-color: rgba(3,170,222,.3); color: var(--white); }
        ...
        .logo-mark { display: inline-flex; align-items: center; gap: 10px; margin-bottom: 6px; }
        .logo-icon { width: 42px; height: 42px; background: linear-gradient(135deg,var(--blue-sky),var(--blue-mid)); border-radius: 12px; display: flex; align-items: center; justify-content: center; box-shadow: 0 8px 20px rgba(3,170,222,.35); }
        .logo-icon i { color: #fff; font-size: 18px; }
        .logo-name { font-family: 'DM Sans', sans-serif; font-weight: 800; font-size: 20px; color: var(--blue-deep); letter-spacing: -.02em; }
        .logo-name span { color: var(--blue-sky); }
        .logo-tagline { font-size: 13px; color: var(--gray-muted); margin-top: 2px; }
        .form-heading { margin-bottom: 28px; }
        .form-heading h2 { font-family: 'Syne', sans-serif; font-size: 26px; font-weight: 700; color: var(--blue-deep); letter-spacing: -.02em; margin-bottom: 4px; }
        .form-heading p { font-size: 14px; color: var(--gray-muted); }
        ...
        .field-input { width: 100%; padding: 13px 14px 13px 42px; background: #F4F7FC; border: 1.5px solid #E2E8F5; border-radius: 12px; font-family: 'DM Sans', sans-serif; font-size: 14px; color: var(--blue-deep); outline: none; transition: all .2s; }
        ...
        .error-box { display: none; background: #FFF0F3; border: 1px solid #FFCED8; border-radius: 10px; padding: 12px 14px; font-size: 13px; color: var(--danger); margin-bottom: 20px; }
        .error-box.show { display: block; }
        .btn-submit { width: 100%; padding: 14px; background: linear-gradient(135deg,var(--blue-sky) 0%,var(--blue-mid) 100%); color: #fff; border: none; border-radius: 12px; font-family: 'Syne', sans-serif; font-size: 15px; font-weight: 700; letter-spacing: .01em; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 8px; transition: all .25s; box-shadow: 0 8px 24px rgba(3,170,222,.35); margin-bottom: 20px; position: relative; overflow: hidden; }
        ...
        .divider::before, .divider::after { content: ''; flex: 1; height: 1px; background: #E2E8F5; }
        .divider span { font-size: 12px; color: #B0BDDA; white-space: nowrap; }
        ...
        /* Modal */
        .modal-overlay { position: fixed; inset: 0; z-index: 99999; display: none; align-items: center; justify-content: center; padding: 20px; background: rgba(8,15,30,.75); backdrop-filter: blur(10px); }
        .modal-overlay.show { display: flex; }
        .modal-card { background: #fff; width: 100%; max-width: 460px; border-radius: 20px; overflow: hidden; box-shadow: 0 30px 80px rgba(0,0,0,.35); animation: modalPop .4s cubic-bezier(.16,1,.3,1) forwards; }
        @keyframes modalPop { from { opacity: 0; transform: scale(.9) translateY(20px); } to { opacity: 1; transform: scale(1) translateY(0); } }
        .modal-top { padding: 28px 28px 24px; background: linear-gradient(135deg,#0D2656,#174EA6); position: relative; overflow: hidden; }
        .modal-top::before { content: ''; position: absolute; top: -40px; right: -40px; width: 140px; height: 140px; background: rgba(255,255,255,.05); border-radius: 50%; }
        .modal-warn-icon { width: 50px; height: 50px; background: rgba(255,183,0,.18); border: 1.5px solid rgba(255,183,0,.4); border-radius: 14px; display: flex; align-items: center; justify-content: center; margin-bottom: 14px; }
        .modal-warn-icon i { color: var(--gold); font-size: 22px; }
        .modal-title { font-family: 'Syne', sans-serif; font-size: 20px; font-weight: 700; color: #fff; margin-bottom: 6px; }
        .modal-desc { font-size: 14px; color: rgba(255,255,255,.6); line-height: 1.5; }
        .modal-body { padding: 20px 28px; }
        .modal-info { display: flex; align-items: center; gap: 10px; background: #EFF6FF; border: 1px solid #DBEAFE; border-radius: 10px; padding: 12px 14px; font-size: 13px; color: #1E40AF; line-height: 1.5; }
        .modal-info i { color: #3B82F6; flex-shrink: 0; }
        .modal-footer { padding: 16px 28px 24px; display: flex; gap: 10px; justify-content: flex-end; }
        .btn-m { padding: 10px 20px; border-radius: 10px; font-family: 'DM Sans', sans-serif; font-size: 14px; font-weight: 500; cursor: pointer; display: flex; align-items: center; gap: 7px; transition: all .2s; border: none; }
        .btn-m-outline { background: #fff; border: 1.5px solid #E2E8F5; color: #374151; }
        .btn-m-outline:hover { background: #F4F7FC; border-color: #C5D0E8; }
        .btn-m-primary { background: linear-gradient(135deg,var(--blue-sky),var(--blue-mid)); color: #fff; box-shadow: 0 4px 16px rgba(3,170,222,.3); }
        .btn-m-primary:hover { opacity: .9; transform: translateY(-1px); }
...
<body>
    ...
    <div id="modal-konflik" class="modal-overlay" role="dialog" aria-modal="true">
        <div class="modal-card">
            <div class="modal-top">
                ...
                <p class="modal-desc">Akun ini sedang aktif di perangkat atau tab lain. Pilih tindakan untuk melanjutkan.</p>
            </div>
    ...
    <div class="page-wrap">
        <div class="left-panel">
            ...
            <p class="hero-sub">Akses materi, tugas, dan evaluasi kapan saja. Wujudkan potensi terbaik Anda bersama Elecomp.</p>
        ...
        async function fetchHandler(email, pass, action = '') {
            ...
            try {
                const fpRes = await fetch(BASE_URL + '/cekfingerprint', {
                    ...
                    body: JSON.stringify({ email, pass, action, fp: getFP() }),
                });
