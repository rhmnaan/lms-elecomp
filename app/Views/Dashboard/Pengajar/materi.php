...
<style>
    ...
    .mp-header h1 { font-size: 20px; font-weight: 800; color: #111; margin: 0 0 3px; }
    .mp-header p  { font-size: 13px; color: #6b7280; margin: 0; }
    ...
    .mp-alert i { font-size: 15px; }

    .mp-toolbar {
        background: #fff; border-radius: 14px;
        padding: 14px 18px; box-shadow: 0 1px 6px rgba(0,0,0,.05);
        margin-bottom: 18px; display: flex; align-items: center; gap: 10px; flex-wrap: wrap;
    }
    .mp-search { position: relative; flex: 1; min-width: 200px; max-width: 280px; }
    .mp-search i { position: absolute; left: 11px; top: 50%; transform: translateY(-50%); color: #9ca3af; font-size: 12px; pointer-events: none; }
    .mp-search input {
        width: 100%; padding: 8px 12px 8px 30px;
        border: 1.5px solid #e5e7eb; border-radius: 9px;
        font-size: 13px; font-family: inherit; color: #374151;
        background: #f9fafb; outline: none; transition: border .18s, background .18s;
    }
    .mp-search input:focus { border-color: #059669; background: #fff; }
    .mp-select {
        ...
        outline: none; cursor: pointer; min-width: 190px; transition: border .18s;
    }
    .mp-select:focus { border-color: #059669; background: #fff; }
    .mp-count { margin-left: auto; font-size: 12px; color: #9ca3af; white-space: nowrap; }
    .mp-count strong { color: #374151; }

    .mp-table-card { background: #fff; border-radius: 16px; box-shadow: 0 1px 8px rgba(0,0,0,.05); overflow: hidden; }
    ...
    .mp-table tbody td { padding: 12px 16px; border-bottom: 1px solid #f9fafb; color: #374151; vertical-align: middle; }
    .mp-table tbody tr:last-child td { border-bottom: none; }
    .mp-table tbody tr:hover td { background: #f9fafb; }

    .mcell { display: flex; align-items: center; gap: 11px; }
    .micon { width: 38px; height: 38px; border-radius: 10px; background: #ecfdf5; color: #059669; display: flex; align-items: center; justify-content: center; font-size: 16px; flex-shrink: 0; }
    .mtitle { font-size: 13px; font-weight: 700; color: #111; margin-bottom: 2px; }
    .mpreview { font-size: 11.5px; color: #9ca3af; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 240px; }

    .media-badges { display: flex; gap: 4px; margin-top: 4px; flex-wrap: wrap; }
    .mbadge { display: inline-flex; align-items: center; gap: 3px; padding: 2px 7px; border-radius: 20px; font-size: 10px; font-weight: 600; white-space: nowrap; }
    .mbadge-pretest  { background: #e0f2fe; color: #0369a1; }
    .mbadge-pdf      { background: #fff1f2; color: #e11d48; }
    .mbadge-video    { background: #fef3c7; color: #d97706; }
    .mbadge-posttest { background: #ede9fe; color: #7c3aed; }
    .badge-modul { display: inline-flex; align-items: center; gap: 5px; padding: 3px 10px; border-radius: 20px; font-size: 11px; font-weight: 600; background: #eff6ff; color: #2563eb; white-space: nowrap; }
    .badge-kelas { display: inline-flex; align-items: center; gap: 5px; padding: 3px 10px; border-radius: 20px; font-size: 11px; font-weight: 600; background: #f0fdf4; color: #059669; white-space: nowrap; }

    .btn-act { display: inline-flex; align-items: center; justify-content: center; width: 31px; height: 31px; border-radius: 7px; border: none; cursor: pointer; font-size: 12px; transition: background .14s, color .14s; text-decoration: none; }
    ...
    .mp-empty { text-align: center; padding: 52px 20px; color: #9ca3af; }
    .mp-empty-icon { width: 66px; height: 66px; border-radius: 18px; background: #f3f4f6; color: #d1d5db; display: inline-flex; align-items: center; justify-content: center; font-size: 28px; margin-bottom: 14px; }
    .mp-empty h3 { font-size: 14px; font-weight: 700; color: #374151; margin-bottom: 4px; }
    .mp-empty p { font-size: 12.5px; margin-bottom: 16px; }
    #mp-no-results { display: none; text-align: center; padding: 44px; color: #9ca3af; font-size: 13px; }
    ...
    .btn-add {
        ...
        transition: transform .14s, box-shadow .14s; white-space: nowrap;
    }
    .btn-add:hover { color: #fff; transform: translateY(-2px); box-shadow: 0 8px 18px rgba(5,150,105,.38); }

    /* ── Tabs ── */
    .mtab-nav { display: flex; gap: 4px; background: #f3f4f6; border-radius: 10px; padding: 4px; margin-bottom: 20px; }
    .mtab-btn {
        flex: 1; padding: 8px 10px; border: none; border-radius: 7px;
        background: transparent; font-size: 12px; font-weight: 600; color: #6b7280;
        cursor: pointer; display: flex; align-items: center; justify-content: center;
        gap: 5px; transition: all .18s;
    }
    .mtab-btn.active { background: #fff; color: #059669; box-shadow: 0 1px 4px rgba(0,0,0,.1); }
    ...
    .upload-zone {
        ...
        transition: border-color .18s, background .18s; background: #fafafa;
    }
    .upload-zone:hover, .upload-zone.dragover { border-color: #059669; background: #f0fdf4; }
    .upload-zone i { font-size: 28px; color: #d1d5db; margin-bottom: 8px; display: block; }
    .upload-zone.has-file i { color: #059669; }
    .upload-zone p { margin: 0; font-size: 13px; color: #6b7280; }
    .upload-zone .file-name { font-weight: 700; color: #374151; font-size: 13px; }
    .upload-zone input[type=file] { display: none; }

    /* ── Video preview ── */
    .video-preview-box { border-radius: 10px; overflow: hidden; background: #000; margin-top: 10px; display: none; aspect-ratio: 16/9; }
    .video-preview-box iframe { width: 100%; height: 100%; border: none; display: block; }

    /* ── Quiz / soal builder ── */
    .quiz-item {
        background: #f9fafb; border: 1px solid #e5e7eb;
        border-radius: 10px; padding: 14px 14px 10px; margin-bottom: 10px; position: relative;
    }
    .quiz-item.is-pretest  { background: #f0f9ff; border-color: #bae6fd; }
    .quiz-item.is-posttest { background: #faf5ff; border-color: #ddd6fe; }
    ...
    .quiz-item.is-pretest .quiz-num  { background: #0ea5e9; }
    .quiz-item.is-posttest .quiz-num { background: #7c3aed; }
    ...
    .quiz-remove:hover { background: #fecaca; }

    .choice-row { display: flex; align-items: center; gap: 8px; margin-bottom: 6px; }
    .choice-row input[type=radio] { flex-shrink: 0; }
    .choice-row.is-pretest  input[type=radio] { accent-color: #0ea5e9; }
    .choice-row.is-posttest input[type=radio] { accent-color: #7c3aed; }
    .choice-row .form-control { flex: 1; font-size: 12px; padding: 6px 10px; }
    ...
    .btn-add-quiz {
        ...
        font-size: 12px; font-weight: 600; cursor: pointer; transition: background .14s;
    }
    .btn-add-quiz:hover { background: #ede9fe; }
    .btn-add-quiz.blue { background: #f0f9ff; color: #0284c7; border-color: #7dd3fc; }
    .btn-add-quiz.blue:hover { background: #e0f2fe; }
    ...
    /* ── Preview quiz list ── */
    .quiz-preview-wrap { margin-bottom: 16px; }
    .quiz-preview-label {
        display: inline-flex; align-items: center; gap: 5px;
        font-size: 11px; font-weight: 700; padding: 3px 10px; border-radius: 20px;
        margin-bottom: 10px;
    }
    .quiz-preview-label.pre  { color: #0369a1; background: #e0f2fe; }
    .quiz-preview-label.post { color: #7c3aed; background: #ede9fe; }

    .quiz-preview-list { list-style: none; padding: 0; margin: 0; }
    .quiz-preview-list > li {
        padding: 10px 14px; border-radius: 8px;
        background: #f8fafc; border: 1px solid #e5e7eb;
        margin-bottom: 8px; font-size: 13px;
    }
    .quiz-preview-list.pre  > li { background: #f0f9ff; border-color: #bae6fd; }
    .quiz-preview-list.post > li { background: #faf5ff; border-color: #ddd6fe; }
    .q-num { font-weight: 800; margin-right: 4px; }
    .q-num.pre  { color: #0369a1; }
    .q-num.post { color: #7c3aed; }
    .choice-list { list-style: none; margin: 6px 0 0 16px; padding: 0; }
    .choice-list li { font-size: 12px; color: #6b7280; padding: 2px 0; }
                                ...
                                <div class="mcell">
                                    ...
                                    <div>
                                        ...
                                        <div class="media-badges">
                                            <?php if (count($preArr) > 0): ?>
                                                <span class="mbadge mbadge-pretest">
                                                    <i class="bi bi-list-check"></i> Pre Test (<?= count($preArr) ?>)
                                                </span>
                                            <?php endif; ?>
                                            ...
                                            <?php if (!empty($mt['video_url_materi'])): ?>
                                                <span class="mbadge mbadge-video"><i class="bi bi-play-circle-fill"></i> Video</span>
                                            <?php endif; ?>
                                            <?php if (count($postArr) > 0): ?>
                                                <span class="mbadge mbadge-posttest">
                                                    <i class="bi bi-patch-question-fill"></i> Post Test (<?= count($postArr) ?>)
                                                </span>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                ...
                <form id="formTambah" action="<?= base_url('dashboard/pengajar/materi/store') ?>" method="POST"
                    ...
                    <div class="mtab-pane" id="tab-video-tambah">
                        ...
                        <div class="input-group mb-2">
                            ...
                            <input type="url" class="form-control border-start-0" name="video_url_materi"
                ...
                <form id="formEdit" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mtab-pane" id="tab-video-edit">
                        ...
                        <div class="input-group mb-2">
                            ...
                            <input type="url" class="form-control border-start-0" name="video_url_materi"
                                id="edit_video_url" placeholder="https://www.youtube.com/watch?v=...">
                        </div>
                        <div class="video-preview-box" id="videoPrevEdit">
                            <iframe id="videoIframeEdit" src="" allowfullscreen></iframe>
                        </div>
                    </div>
        ...
        <div class="modal-content border-0 shadow-lg rounded-4">
            ...
            <div class="modal-body px-4 pt-3 pb-4">
                ...
                <div class="quiz-preview-wrap" id="previewPreWrap">
                    <div class="quiz-preview-label pre">
                        <i class="bi bi-list-check"></i> Pre Test
                    </div>
                ...
                <!-- Video -->
                <div class="video-preview-box mb-3" id="previewVideoBox">
                    <iframe id="previewVideoIframe" src="" allowfullscreen></iframe>
                </div>

                <!-- POST TEST -->
                <div class="quiz-preview-wrap" id="previewPostWrap" style="display:none;">
                    <div class="quiz-preview-label post">
                        <i class="bi bi-patch-question-fill"></i> Post Test
                    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function filterTable() {
        ...
        let n = 1;
        rows.forEach(row => { if (row.style.display !== 'none') row.querySelector('.row-num').textContent = n++; });
    ...
    /* ══════════════════════════════════════
       VIDEO PREVIEW
    ══════════════════════════════════════ */
    function bindVideoPreview(inputId, boxId, iframeId) {
        const input = document.getElementById(inputId);
        if (!input) return;
        input.addEventListener('input', () => {
            const embed = ytEmbed(input.value);
            document.getElementById(iframeId).src = embed || '';
            document.getElementById(boxId).style.display = embed ? 'block' : 'none';
        });
    }
    bindVideoPreview('videoUrlTambah', 'videoPrevTambah', 'videoIframeTambah');
    bindVideoPreview('edit_video_url', 'videoPrevEdit',   'videoIframeEdit');
    ...
    /* Wire tombol Tambah */
    document.getElementById('btnAddPreTambah').addEventListener('click', () =>
        addSoal('pre_test',  'containerPreTambah',  'emptyPreTambah',  true));
    document.getElementById('btnAddPostTambah').addEventListener('click', () =>
        addSoal('post_test', 'containerPostTambah', 'emptyPostTambah', false));

    /* Wire tombol Edit */
    document.getElementById('btnAddPreEdit').addEventListener('click', () =>
        addSoal('pre_test',  'containerPreEdit',  'emptyPreEdit',  true));
    document.getElementById('btnAddPostEdit').addEventListener('click', () =>
        addSoal('post_test', 'containerPostEdit', 'emptyPostEdit', false));
    ...
    function renderQuizList(containerId, soalArr, isPre) {
        ...
        const numCls = isPre ? 'pre'  : 'post';
        let html = `<ul class="quiz-preview-list ${cls}">`;
        soalArr.forEach((s, i) => {
            html += `<li>
                <span class="q-num ${numCls}">Q${i + 1}.</span> ${esc(s.pertanyaan)}
                <ul class="choice-list">`;
            (s.pilihan || []).forEach((p, pi) => {
                const isCorrect = pi == s.jawaban_benar;
                html += `<li class="${isCorrect ? 'correct' : ''}">
                    ${isCorrect ? '✓' : String.fromCharCode(65 + pi) + '.'} ${esc(p)}
                </li>`;
            });
            html += `</ul></li>`;
        });
    ...
    document.querySelectorAll('.btn-hapus-materi').forEach(btn => {
        btn.addEventListener('click', function () {
            document.getElementById('formHapus').action =
                `${BASE_URL}/delete/${this.dataset.rowId}`;
    ...
    /* ══════════════════════════════════════
       CLEAR IFRAMES ON MODAL CLOSE
    ══════════════════════════════════════ */
    ['modalPreview', 'modalEdit', 'modalTambah'].forEach(id => {
